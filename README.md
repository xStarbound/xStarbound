...
3. Install CMake, Git and the required build libraries for xStarbound:
   - *Arch-based distros (CachyOS, Endeavour, etc.):* `sudo pacman -S cmake git ninja mesa libx11 glu libxcb libxrender libxi libxkbcommon libxkbcommon-x11 egl-wayland qt6-svg qt6-base sdl2 libpng freetype2 libvorbis opus` (you may need to `-Syu` first)
   - *RPM/`dnf`-based distros:* `sudo dnf install gcc-c++ cmake git ninja-build mesa mesa-libGLU libXrender libXi libxkbcommon egl-wayland qt6-qtbase qt6-qtsvg SDL2-devel libpng-devel freetype-devel libvorbis-devel opus-devel` (yes, Fedora doesn't come with a C++ compiler preinstalled!)
...
3. Install CMake, Git and the required build libraries for xStarbound:
   ...
   - *RPM/`dnf`-based distros:* `sudo dnf install gcc-c++ cmake git ninja-build patchelf mesa mesa-libGLU libXrender libXi libxkbcommon egl-wayland xcb* libX11-xcb` (yes, Fedora doesn't come with a compiler preinstalled!)

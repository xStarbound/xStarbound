...
3. Install CMake, Git and the required build libraries for xStarbound:
   ...
   - *RPM/`dnf`-based distros:* `sudo dnf install gcc-c++ cmake git ninja-build mesa mesa-libGLU libXrender libXi libxkbcommon egl-wayland qt6-qtbase qt6-qtsvg SDL2-devel libpng-devel freetype-devel libvorbis-devel opus-devel glew-devel` (yes, Fedora doesn't come with a C++ compiler preinstalled!)

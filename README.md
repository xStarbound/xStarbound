...
- Inventory and action bar expansion (or reduction) mods are now fully compatible with vanilla multiplayer with no changes needed on the mod's part. Additionally, these mods can now be safely added or removed without item loss or crashes as long as the player is loaded in xSB-2. Added by WasabiRaptor and FezzedOne.
  ...
  - *Warning for users of vanilla clients and other client mods:* If you load any players that haven't yet dropped overflowed items on a non-xSB-2 client, you may lose items, so load your character in xSB-2 and save your items first!

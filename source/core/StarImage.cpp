...
tuple<Vec2U, PixelFormat> Image::readPngMetadata(IODevicePtr device) {
  ...
  // Fix for memory leak discovered by @kblaschke.
  png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
...
Image& Image::operator=(Image&& image) {
  ...
  m_width = take(image.m_width);             // Added by Kae.
...
void Image::reset(unsigned width, unsigned height, Maybe<PixelFormat> pf) {
  ...
  if (m_width == width && m_height == height && m_pixelFormat == *pf) // Removed broken "fix" by Kae.
    return;
...
Image Image::subImage(Vec2U const& pos, Vec2U const& size) const {
  ...
  uint32_t maxX = m_width  == 0 ? 0 : m_width  - 1,
           maxY = m_height == 0 ? 0 : m_height - 1;
           
  Vec2U baseSizePos = (pos + size).piecewiseMin(Vec2U(maxX, maxY)); 
  Vec2U adjPos = pos.piecewiseMin(Vec2U(maxX, maxY));

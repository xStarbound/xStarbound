...
LuaCallbacks LuaBindings::makePlayerCallbacks(Player* player, bool removeChatCallbacks) {
  ...
  callbacks.registerCallback("setExternalWarpsIgnored", [player](bool ignored) { player->setExternalWarpsIgnored(ignored); });

  callbacks.registerCallback("setExternalRadioMessagesIgnored", [player](bool ignored) { player->setExternalRadioMessagesIgnored(ignored); });

  callbacks.registerCallback("setExternalCinematicsIgnored", [player](bool ignored) { player->setExternalCinematicsIgnored(ignored); });

  callbacks.registerCallback("setPhysicsEntitiesIgnored", [player](bool ignored) { player->setPhysicsEntitiesIgnored(ignored); });

  callbacks.registerCallback("setNudityIgnored", [player](bool ignored) { player->setForcedNudityIgnored(ignored); });

  callbacks.registerCallback("setTechOverridesIgnored", [player](bool ignored) { player->setTechOverridesIgnored(ignored); });

  callbacks.registerCallback("toggleOverreach", [player](bool toggle) { player->setCanReachAll(toggle); });

  callbacks.registerCallback("toggleInWorldRespawn", [player](bool toggle) { player->setAlwaysRespawnInWorld(toggle); });

  callbacks.registerCallback("setIgnoreItemPickups", [player](bool ignore) { player->setIgnoreItemPickups(ignore); });

  callbacks.registerCallback("setIgnoreShipUpdates", [player](bool ignore) { player->setIgnoreShipUpdates(ignore); });

  callbacks.registerCallback("toggleFastWarp", [player](bool ignore) { player->setFastRespawn(ignore); });

  callbacks.registerCallback("externalWarpsIgnored", [player]() -> bool { return player->externalWarpsIgnored(); });

  callbacks.registerCallback("externalRadioMessagesIgnored", [player]() -> bool { return player->externalRadioMessagesIgnored(); });

  callbacks.registerCallback("externalCinematicsIgnored", [player]() -> bool { return player->externalCinematicsIgnored(); });

  callbacks.registerCallback("physicsEntitiesIgnored", [player]() -> bool { return player->physicsEntitiesIgnored(); });

  callbacks.registerCallback("nudityIgnored", [player]() -> bool { return player->forcedNudityIgnored(); });

  callbacks.registerCallback("overreach", [player]() -> bool { return player->canReachAll(); });

  callbacks.registerCallback("techOverridesIgnored", [player]() -> bool { return player->ignoreAllTechOverrides(); });

  callbacks.registerCallback("itemPickupsIgnored", [player]() -> bool { return player->ignoreItemPickups(); });

  callbacks.registerCallback("fastWarp", [player]() -> bool { return player->fastRespawn(); });

  callbacks.registerCallback("inWorldRespawn", [player]() -> bool { return player->alwaysRespawnInWorld(); });

  callbacks.registerCallback("shipUpdatesIgnored", [player]() -> bool { return player->shipUpdatesIgnored(); });

  callbacks.registerCallback("getChatText", [player]() -> String { return player->chatText(); });

  callbacks.registerCallback("chatHasFocus", [player]() -> bool { return player->chatOpen(); });

  callbacks.registerCallback("overrideTypingIndicator", [player](bool overridden) { player->overrideChatIndicator(overridden); });

  callbacks.registerCallback("overrideMenuIndicator", [player](bool overridden) { player->overrideMenuIndicator(overridden); });
  ...
  callbacks.registerCallback("overrideCameraPosition", [player](Maybe<Vec2F> newCameraPosition) { player->overrideCameraPosition(newCameraPosition); });
  ...
  callbacks.registerCallback("setToolUsageSuppressed", [player](Maybe<bool> suppressed) { player->setToolUsageSuppressed(suppressed); });

  callbacks.registerCallback("teamMembers", [player]() -> Json { return player->teamMembers(); });

  callbacks.registerCallback("controlAimPosition", [player](Vec2F const& newAimPosition) { player->aim(std::move(newAimPosition)); });

  callbacks.registerCallback("controlShifting", [player](Maybe<bool> const& shifting) { if (shifting) player->setShifting(*shifting);

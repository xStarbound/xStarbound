...
LuaCallbacks LuaBindings::makeCelestialCallbacks(UniverseClient* client) {
  ...
  auto systemWorldPtr = client->systemWorldClient();
  auto celestialDatabasePtr = client->celestialDatabase();
  auto systemWorld = GameObjectRegistry::smuggleWrap(systemWorldPtr.get());
  auto celestialDatabase = GameObjectRegistry::smuggleWrap(celestialDatabasePtr.get());

  callbacks.registerCallback("skyFlying", [client]() {
    if (auto sky = client->currentSky())
      return sky->flying();
    return false;
  });
  ...
  callbacks.registerCallback("skyWarpPhase", [client]() -> String {
    if (auto sky = client->currentSky())
      return WarpPhaseNames.getRight(sky->warpPhase());
    return "";
  });
  callbacks.registerCallback("skyWarpProgress", [client]() -> float {
    if (auto sky = client->currentSky())
      return sky->warpProgress();
    return 0.0f;
  });
  callbacks.registerCallback("skyInHyperspace", [client]() -> bool {
    if (auto sky = client->currentSky())
      return sky->inHyperspace();
    return false;
  });

  callbacks.registerCallback("flyShip", [client](Vec3I const& system, Json const& destination) {
  ...
  callbacks.registerCallback("centralBodyImages", [celestialDatabase, celestialDatabasePtr](Json const& coords) -> List<pair<String, float>> {
    celestialDatabase.checkSmuggle();
    CelestialCoordinate coordinate = CelestialCoordinate(coords);
    return CelestialGraphics::drawSystemCentralBody(celestialDatabasePtr, coordinate);
  });
  callbacks.registerCallback("planetaryObjectImages", [celestialDatabase, celestialDatabasePtr](Json const& coords) -> List<pair<String, float>> {
    celestialDatabase.checkSmuggle();
    CelestialCoordinate coordinate = CelestialCoordinate(coords);
    return CelestialGraphics::drawSystemPlanetaryObject(celestialDatabasePtr, coordinate);
  });
  callbacks.registerCallback("worldImages", [celestialDatabase, celestialDatabasePtr](Json const& coords) -> List<pair<String, float>> {
    celestialDatabase.checkSmuggle();
    CelestialCoordinate coordinate = CelestialCoordinate(coords);
    return CelestialGraphics::drawWorld(celestialDatabasePtr, coordinate);
  });
  callbacks.registerCallback("starImages", [celestialDatabase, celestialDatabasePtr](Json const& coords, float twinkleTime) -> List<pair<String, float>> {
    celestialDatabase.checkSmuggle();
    CelestialCoordinate coordinate = CelestialCoordinate(coords);
    return CelestialGraphics::drawSystemTwinkle(celestialDatabasePtr, coordinate, twinkleTime);
  });

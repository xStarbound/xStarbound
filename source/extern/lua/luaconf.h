...
// FezzedOne: Disable Pluto's `try`, `catch`, `pluto_try` and `pluto_catch` keywords completely. This is in preparation for when
// Pluto 0.13.0, which removes these keywords, is released in a few months and integrated into xStarbound. (Hopefully, the
// horrible Lua 5.5 GC performance on Windows is fixed by then.)
#define XSB_DISABLE_TRY_CATCH

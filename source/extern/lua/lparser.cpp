...
static void removevars(FuncState* fs, int tolevel) {
  ...
  while (fs->nactvar > tolevel) {
    ...
    if (!vd->vd.used && (vd->vd.kind == VDKREG || vd->vd.kind == RDKTOCLOSE) /* only care if variable is in a register + we don't detect constants being used */
...
static void check_assignment(LexState* ls, const expdesc* v) {
  const auto line = ls->getLineNumber();
  if (v->k == VINDEXUP && (ls->getWarningConfig().states[WT_IMPLICIT_GLOBAL] == WS_UNSPECIFIED ? ls->isKeywordEnabled(TK_GLOBAL) : ls->shouldEmitWarning(line, WT_IMPLICIT_GLOBAL))) {
    luaX_prev(ls);
...
static void const_expr(LexState* ls, expdesc* v) {
  switch (ls->t.token) {
    case TK_NAME: {
      if (!check_constexpr_call(ls, v, "tonumber", luaB_tonumber) && !check_constexpr_call(ls, v, "utonumber", luaB_utonumber) && !check_constexpr_call(ls, v, "tostring", luaB_tostring) && !check_constexpr_call(ls, v, "utostring", luaB_utostring) && !check_constexpr_call(ls, v, "type", luaB_type) && !check_constexpr_call(ls, v, "assert", luaB_assert)) {
...
static void switchimpl(LexState* ls, int tk, void (*caselist)(LexState*, void*), void* ud = nullptr) {
  ...
  expr(ls, &ctrl);
  checknext(ls, TK_DO);
  if (!vkhasregister(ctrl.k) || ctrl.t != ctrl.f /* has jumps? */
  ) {
        ...
        if (var.k == VLOCAL && testnext(ls, ')')) {
          if (testnext(ls, TK_EQ)) {
            if (ls->t.token == TK_STRING) {
              ...
              } else if (!eqstr(ls->t.seminfo.ts, luaX_newliteral(ls, "userdata")) && !eqstr(ls->t.seminfo.ts, luaX_newliteral(ls, "thread")) && !eqstr(ls->t.seminfo.ts, luaX_newliteral(ls, "no value"))) {
...
static void retstat(LexState* ls, TypeHint* prop) {
  ...
  int startpc = fs->pc;
  int endpc = -1;
  if (block_follow(ls, 1) || ls->t.token == ';' || (!ls->switchstates.empty() && (ls->t.token == TK_CASE || ls->t.token == TK_DEFAULT || ls->t.token == TK_BREAK))) {

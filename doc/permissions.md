...
- **`"secureWarps"`:** Whether the server should validate `Player:` and `ClientShipWorld:` warps from non-admin players. If validation is enabled, `Player:` warps from non-admin players are rejected if they don't point to at least one of the originating player, any member of the originating player's team, or any world where the originating player or shared team members are present, and `ClientShipWorld:` warps from non-admin players are rejected if the target shipworld is not the player's own shipworld, a shipworld owned by a shared team member of the player or a shipworld where the player or any shared team members are present. Also logs warp request packets with invalid warp types if enabled (otherwise they're silently ignored). This setting also enables server-side protection against `"warp"` messages sent by non-admin clients to entities they don't control, such as other players, and logs any attempts to do this kind of thing. _Does not require the build permission system to be enabled._
  ...
  - _For admins:_ If you're logged in as an admin, enable `/admin` to bypass this security measure on your client.
